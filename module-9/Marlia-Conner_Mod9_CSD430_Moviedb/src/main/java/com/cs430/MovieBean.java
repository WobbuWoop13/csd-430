...
public static List<MovieBean> getAllMovies() {
    ...
    try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/CSD430", "student1", "pass");
         Statement stmt = conn.createStatement();
         ResultSet rs = stmt.executeQuery(sql)) {
        while (rs.next()) {
...
/**
 * Deletes a movie record from the database by movie_id.
 */
public boolean deleteMovieById(int id) {
    String sql = "DELETE FROM kyle_movies_data WHERE movie_id = ?";
    try (Connection conn = getConnection();
         PreparedStatement ps = conn.prepareStatement(sql)) {
        ps.setInt(1, id);
        return ps.executeUpdate() > 0;
